...
# Initialize logger
logger = get_logger("glide_finetune.master_weight_manager")
...
class MasterWeightManager:
    ...
    def _create_master_weights(self) -> None:
        ...
        for name, param in self.model.named_parameters():
            ...
            if param.dtype == torch.float16:
                ...
                self.logger.debug(
                    f"Created master weight for {name} "
    ...
    def get_param_groups(
        self, base_lr: float = 1e-4, weight_decay: float = 0.0
    ) -> list[dict[str, Any]]:
        """
    ...
    def create_optimizer(self, optimizer_class: type, **optimizer_kwargs) -> torch.optim.Optimizer:
        """
    ...
    def load_state_dict(self, state_dict: dict[str, Any]) -> None:
        ...
        if "master_params" in state_dict:
            for master, saved in zip(self.master_params, state_dict["master_params"], strict=False):
    ...
    def log_stats(self) -> None:
        ...
        self.logger.info("Master Weight Manager Stats:")
...
def test_master_weights():
    ...
    logger.info("Testing Master Weight Manager...")
    ...
    # Check gradients
    logger.info(
        f"Master param gradients created: "
    ...
    logger.info(f"  Max difference: {sync_stats['max_diff']:.6f}")
    logger.info(f"  Mean difference: {sync_stats['mean_diff']:.6f}")
    logger.info(f"  Mismatched params: {sync_stats['mismatched_params']}")
