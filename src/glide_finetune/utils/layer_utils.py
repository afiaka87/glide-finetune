...
# Initialize logger
logger = get_logger("glide_finetune.layer_utils")
...
DEFAULT_ADAPTER_PATTERNS: tuple[str, ...] = ("lora_", "loraA", "loraB", "adapter_", "ada_")
...
def get_component_by_name(model: nn.Module, name: str) -> nn.Module | None:
    """
...
def select_layers_by_mode(
    ...
    if mode == "transformer":
        ...
        excluded_components = [
            c for c in TRANSFORMER_COMPONENTS + DIFFUSION_COMPONENTS if hasattr(model, c)
        ]
...
def apply_to_selected_components(
    ...
    if mode == "transformer":
        ...
        msg = f"Invalid mode: {mode}. Must be 'transformer' or 'diffusion'"
        raise ValueError(msg)

    # Apply the operation to each component
    for component_name, component in components.items():
        operation_fn(component, component_name)
        logger.info(f"  Applied {operation_name} to: {component_name}")
    ...
    logger.info(f"  Components affected: {len(components)}")
    logger.info(
        f"  Parameters affected: {summary.selected_params:,} ({summary.percentage_selected():.1f}%)"
    )
    logger.info(
        f"  Parameters unchanged: {summary.excluded_params:,} ({summary.percentage_excluded():.1f}%)"
    )
...
def validate_mutual_exclusion(
    ...
    if freeze_transformer and randomize_transformer:
        msg = "Cannot both freeze and randomize transformer components"
        raise ValueError(msg)

    if freeze_diffusion and randomize_diffusion:
        msg = "Cannot both freeze and randomize diffusion components"
        raise ValueError(msg)
