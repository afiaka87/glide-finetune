...
from timm.models.layers import DropPath, to_2tuple, trunc_normal_  # type: ignore[attr-defined]
...
# Initialize logger
logger = get_logger("glide_finetune.network_swinir")


class Mlp(nn.Module):
    """Multi-layer perceptron module with dropout."""
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        """
        batch_windows, num_patches, channels = x.shape
        qkv = (
            self.qkv(x)
            .reshape(batch_windows, num_patches, 3, self.num_heads, channels // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
        ...
        if mask is not None:
            ...
            attn = attn.view(batch_windows // num_windows, num_windows, self.num_heads, num_patches, num_patches) + mask.unsqueeze(1).unsqueeze(0)
        ...
        x = (attn @ value).transpose(1, 2).reshape(batch_windows, num_patches, channels)
        x = self.proj(x)
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size: tuple[int, int]) -> torch.Tensor:
        ...
        height, width = x_size
        img_mask = torch.zeros((1, height, width, 1))  # 1 H W 1
        h_slices = (
    ...
    def forward(self, x: torch.Tensor, x_size: tuple[int, int]) -> torch.Tensor:
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, height, width)  # B H' W' C
    ...
    def flops(self) -> int:
        ...
        flops += int(num_windows * self.attn.flops(self.window_size * self.window_size))
        # mlp
        flops += int(2 * height * width * self.dim * self.dim * self.mlp_ratio)
        # norm2
        flops += self.dim * height * width
        return flops
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(batch, -1, 4 * channels)  # B H/2*W/2 4*C
    ...
    def flops(self) -> int:
        """Calculate FLOPs for patch merging."""
        height, width = self.input_resolution
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        if downsample is not None:
            self.downsample: nn.Module | None = downsample(input_resolution, dim=dim, norm_layer=norm_layer)
        else:
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        img_size = to_2tuple(img_size)  # type: ignore[no-untyped-call]
        patch_size = to_2tuple(patch_size)  # type: ignore[no-untyped-call]
...
class PatchUnEmbed(nn.Module):
    ...
    def __init__(
        ...
        in_chans: int = 3,
        embed_dim: int = 96,
        norm_layer: type[nn.Module] | None = None,  # noqa: ARG002 - Interface compatibility
    ) -> None:
        super().__init__()
        img_size = to_2tuple(img_size)  # type: ignore[no-untyped-call]
        patch_size = to_2tuple(patch_size)  # type: ignore[no-untyped-call]
...
class UpsampleOneStep(nn.Sequential):
    ...
    def flops(self) -> int:
        """Calculate FLOPs for upsampling."""
...
class SwinIR(nn.Module):
    ...
    def no_weight_decay(self) -> set[str]:
        """Return set of parameter names that should not have weight decay."""
        return {"absolute_pos_embed"}

    @torch.jit.ignore
    def no_weight_decay_keywords(self) -> set[str]:
        """Return set of parameter keywords that should not have weight decay."""
        return {"relative_position_bias_table"}

    def check_image_size(self, x: torch.Tensor) -> torch.Tensor:
        """Check and pad image size to be divisible by window size."""
        _, _, h, w = x.size()
...
if __name__ == "__main__":
    ...
    logger.info(model)
    logger.info(height, width, model.flops() / 1e9)
