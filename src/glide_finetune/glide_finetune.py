...
from glide_finetune.utils import glide_util, train_util
...
# Initialize logger
logger = get_logger("glide_finetune.glide_finetune")
...
def create_image_grid(
    images: list[PIL.Image.Image], rows: int = 2, cols: int = 4
) -> PIL.Image.Image:
...
def upsample_train_step(
    ...
    noised_high_res_image = glide_diffusion.q_sample(high_res_image, timesteps, noise=noise).to(
        device
    )
    _, channels = noised_high_res_image.shape[:2]
    model_output = glide_model(
...
def base_train_step_freeze_aware(
    ...
    freeze_diffusion: bool = False,  # noqa: ARG001 - Used via model state, not direct conditionals
        ...
        if hasattr(glide_model, "xf_width") and glide_model.xf_width:
            if text_encoder_cache is not None:
                ...
                with th.no_grad():
                    ...
                    xf_out = (
                        text_outputs["xf_out"].detach()
                        if text_outputs["xf_out"] is not None
                        else None
                    )
...
def run_glide_finetune_epoch(
    ...
    _prompt: str = "",  # prompt for inference, not training (deprecated, use test_prompts)
    test_prompts: list[str] | None = None,  # list of prompts for evaluation
    grid_size: int | None = None,  # grid size for visualization
    side_x: int = 64,
    side_y: int = 64,
    outputs_dir: str = "./outputs",
    _checkpoints_dir: str = "./finetune_checkpoints",
    device: str = "cpu",
    ...
    if freeze_transformer and use_text_encoder_cache:
        ...
        logger.info("Created text encoder cache for frozen transformer mode")
    ...
    metrics_tracker = MetricsTracker(
        window_size=log_frequency, gradient_accumulation_steps=gradient_accumulation_steps
    )
    ...
    for train_idx, batch in enumerate(dataloader):
        ...
        log = {
            **log,
            ...
            "optimizer_steps": metrics_tracker.total_optimizer_steps,
        }
        ...
        if train_idx > 0 and train_idx % log_frequency == 0:
            ...
            logger.info(f"[Epoch {epoch} | Iter {train_idx} | OptStep {opt_steps}] {console_summary}")

            # Get all metrics for wandb
            all_metrics = metrics_tracker.get_metrics()
            all_metrics.update({"iter": train_idx, "epoch": epoch, "optimizer_steps": opt_steps})
        ...
        if train_idx > 0 and train_idx % sample_frequency == 0:
            ...
            logger.info(
                f"   📈 Current metrics: Loss={sample_metrics.get('loss', 0):.4f}, Steps/s={sample_metrics.get('steps_per_sec', 0):.2f}"
            )
            ...
            for prompt_idx, test_prompt in enumerate(test_prompts):
                ...
                individual_save_path = os.path.join(
                    outputs_dir, f"iter{train_idx:06d}_prompt{prompt_idx}.png"
                )
            ...
            wandb_log_dict = {
                **log,
                "iter": train_idx,
                "samples_grid": wandb.Image(
                    grid_img, caption=f"{grid_rows}x{grid_cols} Grid of Test Samples"
                ),
            ...
            if sampling_end_time:
                ...
                th.cuda.synchronize()
                sampling_time = (
                    sampling_start_time.elapsed_time(sampling_end_time) / 1000.0
                )  # Convert to seconds
                logger.info(
                    f"✓ Generated {len(sample_images)} samples in {sampling_time:.2f}s, saved grid to {grid_save_path}"
                )
                ...
                logger.info(f"✓ Generated {len(sample_images)} samples, saved grid to {grid_save_path}")
            logger.info()
    ...
    if (train_idx + 1) % gradient_accumulation_steps != 0:
        ...
        logger.info(f"Applied final gradient update for {remaining_steps} accumulated steps")
    ...
    if checkpoint_manager and not checkpoint_manager.interrupted:
        logger.info(f"Finished epoch {epoch}, saving checkpoint")
        checkpoint_manager.save_checkpoint(
            glide_model, optimizer, epoch, final_step, is_interrupted=False
        )
