...
# Initialize logger
logger = get_logger("glide_finetune.text_encoder_cache")
...
class TextEncoderCache:
    ...
    def get(self, tokens: torch.Tensor, mask: torch.Tensor) -> dict[str, torch.Tensor] | None:
        """
        ...
        if tokens.dim() == 2 and tokens.shape[0] == 1:
            ...
            if key in self.cache:
                ...
                return {k: v.clone() if v is not None else None for k, v in self.cache[key].items()}
            self.misses += 1
            return None
        ...
        if batch_results:
            ...
            for key in batch_results[0]:
                if batch_results[0][key] is not None:
                    combined[key] = torch.cat([result[key] for result in batch_results], dim=0)
                else:
                    combined[key] = None
            return combined
    ...
    def put(
        self, tokens: torch.Tensor, mask: torch.Tensor, embeddings: dict[str, torch.Tensor]
    ) -> None:
        """
...
def cached_text_encoder_forward(
    ...
    tokens: torch.Tensor,
    mask: torch.Tensor,
    cache: TextEncoderCache | None = None,
...
class CachedTextEncoder(nn.Module):
    ...
    def forward(self, x_t, timesteps, tokens=None, mask=None):
        ...
        if tokens is not None and mask is not None:
            ...
            cached_text_encoder_forward(self.model, tokens, mask, self.cache)
    ...
    def get_text_emb(self, tokens, mask):
        ...
        return cached_text_encoder_forward(self.model, tokens, mask, self.cache)
...
def test_text_encoder_cache():
    ...
    logger.info("Testing Text Encoder Cache...")
    ...
    # Store in cache
    embeddings = {"xf_proj": torch.randn(1, 512), "xf_out": torch.randn(1, 128, 2048)}
    ...
    # Store batch
    batch_embeddings = {"xf_proj": torch.randn(2, 512), "xf_out": torch.randn(2, 128, 2048)}
    ...
    logger.info("✅ Text Encoder Cache test passed!")
