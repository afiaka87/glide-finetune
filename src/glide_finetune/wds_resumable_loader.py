...
from glide_finetune.utils.glide_util import get_tokens_and_mask
...
# Initialize logger
logger = get_logger("glide_finetune.wds_resumable_loader")
...
def calculate_tar_position(
    ...
    logger.info(f"   Total samples to skip: {total_samples_to_skip:,}")
    logger.info(f"   Starting from tar #{tar_idx + 1}/{len(tar_files)}: {Path(remaining_tars[0]).name}")
    logger.info(f"   Skipping {samples_to_skip_in_tar:,} samples within that tar")
...
def glide_wds_resumable_loader(
    ...
    if isinstance(urls, str):
        tar_files = sorted(glob.glob(urls)) if "*" in urls or "?" in urls or "[" in urls else [urls]
    else:
        tar_files = urls
    ...
    if resume_from_step > 0:
        tar_files, skip_in_first_tar = calculate_tar_position(
            tar_files, resume_from_step, batch_size, gradient_accumulation_steps, samples_per_tar
        )
    ...
    def filter_dataset_alamy(item):
        ...
        if enable_metadata and "json" not in item:
            return False
        json.loads(item["json"].decode("utf-8"))
        return True

    def filter_dataset_synthetic(item):
        return not (enable_image and image_key not in item)
    ...
    def preprocess_dataset(item):
        ...
        if dataset_name == "synthetic":
            ...
            for cap_type in caption_types:
                if cap_type in item:
                    caption = (
                        item[cap_type].decode("utf-8")
                        if isinstance(item[cap_type], bytes)
                        else item[cap_type]
                    )
                    break
            if caption is None:
        ...
        if trim_white_padding:
            original_pil_image = trim_white_padding_pil(
                original_pil_image, white_thresh=white_thresh
            )
    ...
    if skip_in_first_tar > 0:
        logger.info(f"ℹ️  Will skip {skip_in_first_tar} samples in first tar during iteration")
