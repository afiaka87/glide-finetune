...
# Initialize logger
logger = get_logger("glide_finetune.swinir_upscaler")


class UpscaleSR:
    def __init__(
        self, scale: Literal[2, 4, 8] = 4, device: str = "cuda", dtype: torch.dtype = torch.float16
    ) -> None:
    ...
    def __call__(self, imgs_bchw: torch.Tensor) -> torch.Tensor:
        ...
        batch, channels, height, width = imgs_bchw.shape
        imgs01 = (imgs_bchw * 0.5 + 0.5).clamp(0, 1)

        # Process with padding disabled to get exact 4x upscale
        inputs = self.proc(images=imgs01, do_rescale=False, do_pad=False, return_tensors="pt").to(
            self.model.device
        )
        with torch.autocast(
            device_type=self.model.device.type, enabled=self.model.dtype == torch.float16
        ):
...
def create_swinir_upscaler(
    ...
    if model_type not in model_configs:
        logger.warning(f"Warning: Unknown model type '{model_type}', using classical_sr_x4")
    ...
    class ConfiguredUpscaleSR(UpscaleSR):
        def __init__(self, device: str = "cuda", dtype: torch.dtype = torch.float16) -> None:
            ...
            self.scale = cast("Literal[2, 4, 8]", scale)  # Safe cast since we know scale comes from model_configs
            self.proc = AutoImageProcessor.from_pretrained(repo)
            self.model = (
                Swin2SRForImageSuperResolution.from_pretrained(repo)
                .to(device=device, dtype=dtype)
