...
class EMA(nn.Module):
    ...
    def update(self):
        if not self.training:
            msg = "Update should only be called during training"
...
def model_grads_to_master_grads(param_groups_and_shapes, master_params):
    ...
    """
    for master_param, (param_group, shape) in zip(master_params, param_groups_and_shapes, strict=False):
...
def state_dict_to_master_params(model, state_dict, use_fp16):
    if use_fp16:
        ...
        master_params = [
            state_dict[name] for name, param in model.named_parameters() if param.requires_grad
        ]
    return master_params
