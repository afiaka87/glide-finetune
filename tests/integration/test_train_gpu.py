    ...
    def test_train_coco_style_dataset_with_8bit_and_tf32(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--log_frequency", "10",  # Log frequently
                "--use_8bit_adam",  # Enable 8-bit Adam
                "--test_prompt", "a colorful bird",
                "--test_batch_size", "1",
                "--test_guidance_scale", "4.0",
                "--early_stop", "100",  # Stop after 100 steps
            ]
            ...
            assert "Found" in output and "images" in output, "Dataset loading message not found"
            assert "Using" in output and "text files" in output, "Caption loading message not found"
            
            # Check that model was loaded
            assert "Model loaded" in output or "Loading model" in output or "Downloading" in output, \
            ...
            assert "Early stopping at step" in output, "Early stopping message not found"
    ...
    def test_memory_efficient_training(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--test_prompt", "a small bird",
                "--test_batch_size", "1",
                "--early_stop", "50",  # Stop after 50 steps
            ]
            ...
            assert result.returncode == 0, f"Memory-efficient training failed: {result.stderr}"
    ...
    def test_early_stop_exact_count(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                "--test_prompt", "a bird",
                "--test_batch_size", "1",
                "--early_stop", str(early_stop_steps),
            ]
