...
class TestCheckpointResume:
    ...
    def test_resume_from_checkpoint(self, mock_trange, mock_wandb, temp_dirs):
        ...
        run_dirs = [d for d in os.listdir(checkpoint_dir) if os.path.isdir(os.path.join(checkpoint_dir, d))]
        assert len(run_dirs) == 1
        run_dir = os.path.join(checkpoint_dir, run_dirs[0])
        
        # Find checkpoint files
        checkpoint_files = [f for f in os.listdir(run_dir) if f.endswith('.pt') and not f.endswith('.optimizer.pt')]
