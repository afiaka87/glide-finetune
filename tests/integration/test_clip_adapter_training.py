...
def test_clip_adapter_training_improves_loss():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for step in range(num_steps):
            ...
            print(
                f"Step {step}: loss={loss.item():.4f}, gate={current_gate:.4f}, "
        ...
        improvement_found = (late_window < early_window) or (
            min_loss_window < early_window * 0.9
        )
        ...
        assert (non_zero_steps > 0).all(), (
            f"Some steps had zero gradients: {grad_norms_array}"
        )
        ...
        print(
            f"Loss improved from {early_window:.4f} to {late_window:.4f} (best window: {min_loss_window:.4f})"
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for step in range(num_steps):
            ...
            if degradation > degradation_threshold:
                ...
                print(
                    f"Step {step}: Degradation detected! {degradation * 100:.1f}% worse than baseline"
                )
                ...
                if degradation_counter >= patience:
                    ...
                    stop_step = step
                    print(
                        f"Step {step}: Early stopping triggered after {patience} steps of degradation"
                    )
                    break
            else:
            ...
            print(
                f"Step {step}: loss_clip={loss_with_clip.item():.4f}, "
                f"loss_baseline={loss_without_clip.item():.4f}, "
                f"degradation={degradation * 100:.1f}%"
            )

        # Verify early stopping was triggered
        assert early_stop_triggered, (
            "Early stopping was not triggered despite degradation"
        )
        assert stop_step is not None, "Stop step was not recorded"
        assert stop_step >= 5 + patience - 1, "Early stopping triggered too early"
        assert stop_step < num_steps - 1, (
            "Training ran to completion instead of stopping early"
        )

        print("\nEarly stopping test passed!")
        print(
            f"Training stopped at step {stop_step} after {patience} steps of degradation"
        )
        print(
            f"Average degradation when using wrong CLIP: "
...
def test_three_phase_training_transitions():
    ...
    def get_param_gradient_status(model):
        ...
        param_groups = {"adapter": [], "gates": [], "encoder": [], "other": []}
    ...
    print(
        f"  Adapter params with gradients: {adapter_with_grad}/{len(grad_status['adapter'])}"
    )
    print(
        f"  Gate params with gradients: {gates_with_grad}/{len(grad_status['gates'])}"
    )
    print(
        f"  Encoder params with gradients: {encoder_with_grad}/{len(grad_status['encoder'])}"
    )
    print(
        f"  Other params with gradients: {other_with_grad}/{len(grad_status['other'])}"
    )
    ...
    assert adapter_with_grad > 0, (
        "No adapter parameters have gradients in adapter_only phase"
    )
    assert encoder_with_grad == 0, (
        "Encoder parameters should not have gradients (frozen)"
    )
    ...
    print("  Note: 'adapter_only' includes MLP + gates in current implementation")
    ...
    print(
        f"  Note: Gates are included in adapter params, so gates count is {info2['param_counts']['gates']}"
    )
    ...
    assert adapter_with_grad > 0, (
        "No adapter parameters have gradients in adapter_gates phase"
    )
    assert encoder_with_grad == 0, "Encoder parameters should remain frozen"
    print(
        "  Note: adapter_gates phase currently same as adapter_only due to implementation"
    )
    ...
    assert adapter_with_grad > 0, (
        "Adapter parameters should have gradients in full phase"
    )
    assert gates_with_grad > 0 or other_with_grad > 0, (
        "Main model parameters should have gradients in full phase"
    )
    assert encoder_with_grad == 0, (
        "Encoder should remain frozen (freeze_glide_encoder=True)"
    )
    ...
    assert adapter_lr != main_lr, (
        f"Adapter and main should have different LRs: {adapter_lr} vs {main_lr}"
    )

    print("\nThree-phase training transitions test passed!")
    print("Successfully transitioned through adapter_only → adapter_gates → full")
    print("Each phase correctly updated only the intended parameters")
    print("Training remained stable throughout all transitions")
...
def test_visual_quality_improvement():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for step in range(num_train_steps):
            ...
            if step % 10 == 0:
                print(
                    f"Training step {step}: loss={loss.item():.4f}, gate={clip_model.get_stability_metrics()['adapter_gate']:.4f}"
                )
    ...
    for prompt in test_prompts:
        ...
        with torch.no_grad():
            ...
            baseline_features = baseline_features / baseline_features.norm(
                dim=-1, keepdim=True
            )
            ...
            print(
                f"  Improvement: {(clip_score - baseline_score):.4f} ({(clip_score / baseline_score - 1) * 100:.1f}%)"
            )
    ...
    print(
        f"Average difference: {(avg_clip - avg_baseline):.4f} ({(avg_clip / avg_baseline - 1) * 100:.1f}%)"
    )
    ...
    print(
        f"Loss reduction: {(early_losses - late_losses):.4f} ({(1 - late_losses / early_losses) * 100:.1f}%)"
    )
    ...
    print("✓ CLIP scores in expected range (0.1-0.3)")
    ...
    print("the CLIP adapter architecture works correctly. Actual quality")
    print("improvement requires pretrained weights and longer training.")
