...
class MockGlideModel(nn.Module):
    ...
    def __init__(self, device="cpu"):
        ...
        self.tokenizer.padded_tokens_and_mask = Mock(return_value=([1, 2, 3, 0], [True, True, True, False]))
    ...
    def forward(self, x, timesteps, tokens=None, mask=None, low_res=None):
        ...
        output = torch.randn(batch_size, channels * 2, height, width, requires_grad=True)
...
class TestGlideFinetuneComponents:
    ...
    def test_update_metrics(self):
        ...
        updated_log = update_metrics(
            log, loss, step_metrics, 
            global_step=100, train_idx=50, current_lr=1e-4,
            batch_size=4, gradient_accumualation_steps=1,
            glide_model=model
        )
...
class TestGlideFinetuneEpoch:
    ...
    def test_run_epoch_basic(self, temp_dirs, mock_wandb, mock_sample, mock_pred_to_pil):
    ...
    def test_run_epoch_with_warmup(self, temp_dirs, mock_wandb, mock_sample, mock_pred_to_pil):
        ...
        logged_lrs = [call[0][0]["lr"] for call in wandb_run.log.call_args_list if "lr" in call[0][0]]
    ...
    def test_checkpoint_integration(self, temp_dirs, mock_wandb, mock_sample, mock_pred_to_pil):
        ...
        pt_files = [f for f in files if f.endswith('.pt') and not f.endswith('.optimizer.pt')]
