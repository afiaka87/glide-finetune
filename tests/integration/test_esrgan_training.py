...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="GPU required for ESRGAN test")
def test_esrgan_training_vram():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert len(esrgan_images) > 0, "Should have generated ESRGAN upsampled images"
            ...
            for base_img_path in base_images:
                ...
                assert base_img.size == (64, 64), f"Base image should be 64x64, got {base_img.size}"
                ...
                if esrgan_path.exists():
                    ...
                    assert esrgan_img.size == (256, 256), f"ESRGAN image should be 256x256, got {esrgan_img.size}"
                    print(f"✓ Verified {base_img_path.name} (64x64) -> {esrgan_path.name} (256x256)")
            ...
            assert esrgan_overhead < 6.0, f"ESRGAN overhead too high: {esrgan_overhead:.2f} GB"
...
def test_esrgan_memory_efficiency():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Check output shape
        assert upsampled.shape == (4, 3, 256, 256), f"Expected (4, 3, 256, 256), got {upsampled.shape}"
        ...
        assert esrgan_model_size < 0.1, f"ESRGAN model too large: {esrgan_model_size:.3f} GB"
        
        # Upsampling overhead should be reasonable (less than 1GB for batch of 4)
        assert upsample_memory < 1.0, f"Upsampling uses too much memory: {upsample_memory:.3f} GB"
