...
class TestAllSamplers:
    ...
    def test_sampler_quality_metrics(self, test_model_and_diffusion, device, sampler_name, num_steps):
        ...
        # Quality checks
        assert avg_std > 0.05, f"{sampler_name} produced low variance images (std={avg_std:.3f})"
        assert -0.5 < avg_mean < 0.5, f"{sampler_name} produced biased images (mean={avg_mean:.3f})"
    ...
    def test_all_samplers_with_prompt(self, test_model_and_diffusion, device, prompt, tmp_path):
        ...
        good_quality_count = sum(1 for r in results.values() if r.get("quality") == "good")
        assert good_quality_count >= 4, f"Only {good_quality_count}/6 samplers produced good quality"
    ...
    def test_sampler_determinism(self, test_model_and_diffusion, device):
        ...
        for sampler_name, num_steps in self.sampler_configs:
            ...
            assert diff < 1e-5, f"{sampler_name} is not deterministic (max diff: {diff})"
    ...
    def test_sampler_registry(self):
        ...
        assert not any("_fixed" in s for s in registered), "Found '_fixed' samplers in registry"
    ...
    def test_no_scale_model_input(self, test_model_and_diffusion, device, sampler_name):
        ...
        with patch('glide_finetune.samplers.util.scale_model_input', side_effect=RuntimeError("scale_model_input should not be called")):
    ...
    def test_visual_comparison_grid(self, test_model_and_diffusion, device, tmp_path):
        ...
        for prompt in prompts:
            ...
            for sampler in samplers:
                ...
                num_steps = 100 if sampler == "plms" else (25 if "dpm" in sampler else 50)
        ...
        for sampler_name in samplers:
            ...
            for prompt_idx, prompt in enumerate(test_prompts):
                ...
                try:
                    ...
                    print(f"    ✓ Success - std: {std_val:.3f}, mean: {mean_val:.3f} [{result['quality']}]")
