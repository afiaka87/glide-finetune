...
class TestSamplersVisual:
    ...
    @pytest.mark.parametrize("sampler_name,num_steps", [
        ("plms", 100),
        ("ddim", 50),
        ("euler", 50),
        ("euler_a", 50),
    ...
    def test_sampler_visual_quality(self, test_model_and_diffusion, device, sampler_name, num_steps, tmp_path):
        ...
        # Check value ranges
        assert samples.min() >= -1.5, f"{sampler_name} produced values too low: {samples.min()}"
        assert samples.max() <= 1.5, f"{sampler_name} produced values too high: {samples.max()}"
        ...
        assert avg_std > 0.05, f"{sampler_name} produced images with too low variance (avg std: {avg_std:.3f})"
        
        # Images should not be too dark or too bright on average
        assert 0.2 < avg_mean < 0.8, f"{sampler_name} produced images with unusual brightness (avg mean: {avg_mean:.3f})"
        ...
        assert noise_ratio < 1.0, f"{sampler_name} may be producing pure noise (noise ratio: {noise_ratio:.3f})"
...
class TestSamplersVisualWithRealPrompts:
    ...
    def test_diverse_prompts(self, test_model_and_diffusion, device, prompt, sampler_name, tmp_path):
        ...
        # Check quality
        img_array = np.array(img) / 255.0
        assert img_array.std() > 0.05, f"Low variance for prompt '{prompt}' with {sampler_name}"
        assert 0.1 < img_array.mean() < 0.9, f"Unusual brightness for prompt '{prompt}' with {sampler_name}"