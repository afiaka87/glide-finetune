...
class TestClipKLDivergence:
    ...
    def test_kl_divergence_with_nonzero_gate(self, model_and_diffusion):
        ...
        model.set_adapter_gate_schedule(
            8000, 10000
        )  # Sets gate to 0.4 for stronger effect
        ...
        with torch.no_grad():
            ...
            if test_tokens.shape[1] < 128:
                test_tokens = torch.nn.functional.pad(
                    test_tokens, (0, 128 - test_tokens.shape[1])
                )
            test_mask = torch.ones_like(test_tokens).bool()
            test_output = model(test_x, test_t, tokens=test_tokens, mask=test_mask)
            print(
                f"  Test output: mean={test_output.mean().item():.6f}, std={test_output.std().item():.6f}, abs_max={test_output.abs().max().item():.6f}"
            )
        ...
        print(
            f"  clip_embeddings shape: {clip_embeddings.shape}, dtype: {clip_embeddings.dtype}"
        )
        print(
            f"  clip_embeddings norm: {clip_embeddings.norm(dim=-1).mean().item():.4f}"
        )
        print(
            f"  Current gate value: {model.get_stability_metrics().get('adapter_gate', 0)}"
        )
        ...
        with torch.no_grad():
            ...
            eps_with_clip, _ = torch.split(
                output_with_clip, output_with_clip.shape[1] // 2, dim=1
            )
            print(
                f"  output_with_clip: shape={output_with_clip.shape}, has_nan={torch.isnan(output_with_clip).any().item()}, mean={output_with_clip.mean().item():.4f}, std={output_with_clip.std().item():.4f}"
            )
            print(
                f"  eps_with_clip: mean={eps_with_clip.mean().item():.6f}, std={eps_with_clip.std().item():.6f}"
            )
            ...
            # Split to get epsilon prediction
            eps_without_clip, _ = torch.split(
                output_without_clip, output_without_clip.shape[1] // 2, dim=1
            )
            print(
                f"  output_without_clip: shape={output_without_clip.shape}, has_nan={torch.isnan(output_without_clip).any().item()}, mean={output_without_clip.mean().item():.4f}, std={output_without_clip.std().item():.4f}"
            )
            print(
                f"  eps_without_clip: mean={eps_without_clip.mean().item():.6f}, std={eps_without_clip.std().item():.6f}"
            )
            ...
            print(
                f"  Outputs identical: {torch.allclose(output_with_clip, output_without_clip)}"
            )
    ...
    def test_train_step_with_kl_loss(self, model_and_diffusion):
        ...
        loss_no_kl, metrics_no_kl = base_train_step(
            model, diffusion, batch, device, compute_kl_loss=False, kl_loss_weight=0.0
        )

        # Run training step with KL loss
        loss_with_kl, metrics_with_kl = base_train_step(
            model, diffusion, batch, device, compute_kl_loss=True, kl_loss_weight=0.1
        )
