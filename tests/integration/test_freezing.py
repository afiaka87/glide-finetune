...
class TestFreezing:
    ...
    def verify_parameter_counts(self, model, expected_trainable_ratio):
        ...
        print(
            f"  Frozen: {frozen_params:,} ({frozen_params / total_params * 100:.1f}%)"
        )
    ...
    def train_steps(self, model, diffusion, optimizer):
        ...
        for step in range(self.steps):
            ...
            # Model prediction
            model_output = model(
                noised_images, t, tokens=batch["tokens"], mask=batch["mask"]
            )
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                assert param.grad is not None, (
                    f"No gradient for trainable parameter: {name}"
                )
            else:
                assert param.grad is None, (
                    f"Gradient computed for frozen parameter: {name}"
                )
