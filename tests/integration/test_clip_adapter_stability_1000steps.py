...
def test_1000_steps_gate_zero_stability():
    ...
    assert attention_gate_mean < 1e-4, f"Attention gates should be near 0, got {attention_gate_mean}"
    ...
    if num_identical >= 990:
        print(f"✓ TEST PASSED: {num_identical/10:.1f}% of outputs are identical (>= 99%)")
    else:
        print(f"✗ TEST FAILED: Only {num_identical/10:.1f}% of outputs are identical (< 99%)")
    ...
    dry_run_results = clip_model.dry_run_test(
        x, timesteps, tokens=tokens, mask=mask,
        clip_text_prompts=test_prompts
    )
    
    print(f"Dry-run mode results:")
