...
class TestClipDryRun:
    ...
    def test_dry_run_basic(self, model_and_diffusion):
        ...
        assert results['outputs_identical'], f"Outputs not identical with gate=0! Max diff: {results['output_diff_max']}"
    ...
    def test_dry_run_with_nonzero_gate(self, model_and_diffusion):
        ...
        assert results['outputs_identical'], f"Outputs not identical in dry run! Max diff: {results['output_diff_max']}"
    ...
    def test_dry_run_vs_normal_mode(self, model_and_diffusion):
        ...
        assert diff.max().item() > 1e-6, "Normal mode should differ from baseline with gate=0.5"
