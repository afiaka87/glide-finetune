...
class TestClipAdapterStability:
    ...
    def test_gate_zero_produces_identical_outputs(self, models_and_diffusion):
        ...
        assert metrics.get('attention_gate_mean', 0.0) < 1e-4  # sigmoid(-10) ≈ 0.0000454
        ...
        for i in range(10):
            ...
            with torch.no_grad():
                ...
                clip_out_with_emb = clip_model(x, timesteps, tokens=tokens, mask=mask, 
    ...
    def test_gate_zero_with_different_timesteps(self, models_and_diffusion):
        ...
        for t_min, t_max in timestep_ranges:
            ...
            with torch.no_grad():
                ...
                if torch.isnan(clip_out_no_emb).any() or torch.isnan(clip_out_with_emb).any():
                    continue
                
                assert torch.allclose(clip_out_no_emb, clip_out_with_emb, rtol=1e-4, atol=1e-5), \
    ...
    def test_dry_run_mode_matches_baseline(self, models_and_diffusion):
        ...
        results = clip_model.dry_run_test(
            ...
            clip_text_prompts=prompts
        )
        
        # Skip test if outputs are NaN
        if results.get('dry_run_output') is not None and torch.isnan(results['dry_run_output']).any():
    ...
    def test_gate_schedule_at_zero(self, models_and_diffusion):
        ...
        with torch.no_grad():
            output = clip_model(x, timesteps, tokens=tokens, mask=mask,
                              clip_embeddings=clip_model.get_clip_text_emb([prompt]))
