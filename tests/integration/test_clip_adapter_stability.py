...
class TestClipAdapterStability:
    ...
    def test_gate_zero_identity(self, device, tokenizer, sample_prompts, model_options):
        ...
        baseline_model = create_model(
            ...
            attention_resolutions=",".join(map(str, [model_options["image_size"]//res for res in model_options["attention_resolutions"]])),
        ...
        for prompt in sample_prompts:
            # Prepare inputs
            tokens, mask = get_tokens_and_mask(tokenizer, prompt, model_options["text_ctx"])
            tokens = tokens.unsqueeze(0).to(device)
            mask = mask.unsqueeze(0).to(device)
            
            # Random noise and timestep
            x = torch.randn(1, 3, model_options["image_size"], model_options["image_size"]).to(device)
            t = torch.tensor([500], device=device)  # Mid-range timestep
            
            # Forward pass through both models
            with torch.no_grad():
                baseline_out = baseline_model(x, t, tokens=tokens, mask=mask)
                clip_out = clip_model(x, t, tokens=tokens, mask=mask, clip_text_prompts=[prompt])
    ...
    def test_adapter_parameter_isolation(self, device, tokenizer, model_options):
        ...
        expected_keywords = ['clip_adapter', 'conditioning_adapter', 'clip_gate', 'clip_kv']
        ...
        text_encoder_params = ['transformer', 'token_embedding', 'positional_embedding', 
                               'padding_embedding', 'transformer_proj', 'final_ln']
        
        for name, param in model.named_parameters():
            for encoder_param in text_encoder_params:
                if encoder_param in name and 'clip' not in name:
                    assert not param.requires_grad, f"Text encoder param {name} is not frozen"
