...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="File containing list of tar files (one per line)",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
    ...
    # Validate arguments
    if not args.tar_urls and not args.tar_urls_file:
        print("ERROR: Must specify either --tar_urls or --tar_urls_file")
        return 1
    
    if args.tar_urls and args.tar_urls_file:
        print("ERROR: Cannot specify both --tar_urls and --tar_urls_file")
        return 1

    # Get tar file list
    if args.tar_urls_file:
        # Read tar files from file
        with open(args.tar_urls_file, 'r') as f:
            tar_files = [line.strip() for line in f if line.strip()]
        tar_files = sorted(tar_files)
        pattern_info = f"file: {args.tar_urls_file}"
    else:
        # Expand glob pattern to get tar file list
        tar_files = sorted(glob(args.tar_urls))
        pattern_info = f"pattern: {args.tar_urls}"
    
    if not tar_files:
        print(f"No tar files found from {pattern_info}")
        return 1
