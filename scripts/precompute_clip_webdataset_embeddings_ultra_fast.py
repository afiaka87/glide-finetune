...
def load_clip_model_compiled(clip_model_name: str, device: str):
    ...
    model, preprocess = clip.load(clip_model_name, device=device, jit=False)  # JIT incompatible with compile
    model.eval()
...
async def process_tar_file_async(
    ...
    pbar = tqdm(desc=f"   Samples", unit=" samples", unit_scale=True, total=estimated_total,
    ...
    print(f"   ✅ Completed: {total_samples:,} samples in {elapsed:.1f}s ({throughput:,.1f} samples/s)")
...
async def process_all_tars_concurrent(
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            async with semaphore:
                ...
                try:
                    ...
                    tar_metadata[tar_name] = {
                        ...
                        "cache_file": str(cache_file.relative_to(embeddings_dir.parent)),
        ...
        tasks = [process_with_semaphore(tar_path, idx+1) for idx, tar_path in enumerate(tar_files)]
...
def main():
    ...
    if total_samples > 0:
        ...
        for size, count in [("1M", 1_000_000), ("10M", 10_000_000), ("100M", 100_000_000)]:
