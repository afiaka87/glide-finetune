...
def process_batch(
    batch: TarBatch, model, tokenize, device: str, cache_dir: str, normalize: bool
) -> int:
    ...
    output_file = os.path.join(
        cache_dir, f"{batch.tar_name[:-4]}_batch_{batch.batch_start:06d}.npz"
    )

    try:
        np.savez_compressed(output_file, embeddings=embeddings, indices=batch.indices)
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--clip_model_name", type=str, default="ViT-B/32", help="CLIP model to use"
    )
    parser.add_argument(
        "--caption_key",
        type=str,
    ...
    parser.add_argument(
        "--batch_size", type=int, default=2048, help="Batch size for CLIP encoding"
    )
    parser.add_argument(
        "--prefetch_size", type=int, default=3, help="Number of tar files to prefetch"
    )
    parser.add_argument(
        "--normalize", action="store_true", help="Normalize embeddings to unit vectors"
    )
    parser.add_argument(
        "--device",
        type=str,
