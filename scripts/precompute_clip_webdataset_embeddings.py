...
This script processes WebDataset tar files, extracts text captions, encodes them
with CLIP,
        ...
        for sample in dataset:
            ...
            if len(batch_texts) >= batch_size:
                if not dry_run:
                    ...
                    for i, (key, embedding) in enumerate(
                        zip(batch_keys, batch_embeddings)
                    ):
                        embeddings[key] = {
                            "embedding": embedding,
...
def encode_texts_batch(
    model, texts: List[str], device: str, max_length: int = 77
) -> torch.Tensor:
...
def check_existing_cache(
    cache_file: Path, clip_model_name: str, force_recompute: bool = False
) -> bool:
...
def save_embeddings_cache(
    ...
    data = {
        "metadata": {
            ...
            "embedding_dim": embeddings[next(iter(embeddings))]["embedding"].shape[-1]
            if embeddings
            else 0,
        },
...
def precompute_webdataset_embeddings(
    ...
    if not dry_run:
        embeddings_dir, metadata_file = setup_cache_directory(
            cache_path, clip_model_name
        )
...
def main():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=32, help="Batch size for CLIP encoding"
    )
    parser.add_argument(
        "--device",
        type=str,
