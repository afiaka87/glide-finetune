...
class TextEmbedding(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        w_voc = torch.empty(
            (self.n_vocab, self.n_state), dtype=torch.float32, device=self.device
        )
        w_pos = torch.empty(
            (self.n_context, self.n_state), dtype=torch.float32, device=self.device
        )
...
class ImageEmbedding(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        patch_proj = torch.empty(
            (self.n_state, 3) + 2 * (self.patch_size,),
            dtype=torch.float32,
            device=self.device,
        )
        w_pos = torch.empty(
    ...
    def forward(
        ...
        if (self.n_timestep == 0 and t is not None) or (
            self.n_timestep != 0 and t is None
        ):
            raise ValueError()
        if self.n_timestep != 0:
        ...
        x = x.reshape(
            x.shape[0], self.n_state, (self.image_size // self.patch_size) ** 2
        ).permute(0, 2, 1)
...
class AttentionResblock(nn.Module):
    ...
    def forward(self, m: torch.Tensor) -> torch.Tensor:
        ...
        q = q.view([q.shape[0], -1, self.attn_fn.n_heads, self.n_head_state]).permute(
            (0, 2, 1, 3)
        )
        k = k.view([k.shape[0], -1, self.attn_fn.n_heads, self.n_head_state]).permute(
            (0, 2, 1, 3)
        )
        v = v.view([v.shape[0], -1, self.attn_fn.n_heads, self.n_head_state]).permute(
            (0, 2, 1, 3)
        )
        w = torch.einsum(
            "bhcd,bhkd->bhck",
            q * math.sqrt(self.qk_scale),
            k * math.sqrt(self.qk_scale),
        )
...
class ImageFeatureExtractor(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, return_probe_features: bool = False
    ) -> torch.Tensor:
        if return_probe_features:
            return x[:, 0]
...
class TextEncoder(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        n_rounded_context = self.block_size * int(
            math.ceil(self.max_text_len / self.block_size)
        )
        ...
        for i in range(self.n_xf_blocks):
            blocks.append(
                (
                    f"block_{i}",
                    TransformerBlock(
                        self.n_state, 2 * self.n_xf_blocks, attn_fn, self.device
                    ),
                )
            )
        ...
        blocks.append(
            (
                "output",
                TextFeatureExtractor(self.n_state, self.n_embd, device=self.device),
            )
        )
    ...
    def forward(
        ...
        h = self.blocks["output"](
            h, text_len, return_probe_features=return_probe_features
        )
...
class ImageEncoder(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        n_rounded_context = self.block_size * int(
            math.ceil(self.n_context / self.block_size)
        )
        ...
        blocks.append(
            ("output", ImageFeatureExtractor(self.n_state, self.n_embd, self.device))
        )
