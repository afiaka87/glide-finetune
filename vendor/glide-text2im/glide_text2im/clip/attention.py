...
class AttentionMask(ABC):
    ...
    def _make_global_layout(self) -> None:
        if not self.is_head_specific:
            ...
            m = np.ones(
                [self.n_head, self.n_query_block, self.n_key_block], dtype=np.bool
            )
            r = product(*[range(n) for n in m.shape])
    ...
    def block_layout(
        ...
        if query_idx >= self.first_pad_query_block_idx:
            n_pad = min(
                self.block_size,
                (query_idx + 1) * self.block_size
                - (self.query_context_size - self.n_query_pad),
            )
            assert n_pad > 0
        ...
        if key_idx >= self.first_pad_key_block_idx:
            n_pad = min(
                self.block_size,
                (key_idx + 1) * self.block_size
                - (self.key_context_size - self.n_key_pad),
            )
            assert n_pad > 0
            m[:, self.block_size - n_pad :] = False
...
class DenseAttentionMask(AttentionMask):
    def __attrs_post_init__(self) -> None:
        ...
        self.global_layout = np.ones(
            [self.n_query_block, self.n_key_block], dtype=np.bool
        )
...
class DenseCausalAttentionMask(AttentionMask):
    def __attrs_post_init__(self) -> None:
        ...
        self.global_layout = np.tril(
            np.ones([self.n_query_block, self.n_key_block], dtype=np.bool)
        )
        n_zero_query_blocks = self.n_query_pad // self.block_size
        n_zero_key_blocks = self.n_key_pad // self.block_size
        self.global_layout[self.n_query_block - n_zero_query_blocks :] = False
...
def make_full_layout(d: AttentionMask) -> np.ndarray:
    ...
    if not d.is_head_specific:
        ...
        if len(d.global_layout.shape) == 2:
            ...
            u = np.reshape(
                d.global_layout, [d.n_head, d.n_query_block, d.n_key_block, 1, 1]
            )
        else:
            raise RuntimeError()
        ...
        v = np.reshape(
            v, [d.n_head, d.n_query_block, d.n_key_block, d.block_size, d.block_size]
        )
