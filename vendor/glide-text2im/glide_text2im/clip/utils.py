...
class LayerNorm(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        super().__init__()
        self.g = nn.Parameter(
            torch.ones((self.n_state,), dtype=torch.float32, device=self.device)
        )
        self.b = nn.Parameter(
            torch.zeros((self.n_state,), dtype=torch.float32, device=self.device)
        )
...
class Affine(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if not self.use_admnet_init:
            self.std = (
                ...
                else math.sqrt(2 / (self.n_in + self.n_out))
            )
            self.std = (
                self.std
                if self.extra_init_scale is None
                else self.std * self.extra_init_scale
            )

            w = torch.empty(
                (self.n_out, self.n_in), dtype=torch.float32, device=self.device
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        b = (
            self.bias_filter_fn(
                self.b if self.b.dtype == x.dtype else self.b.to(x.dtype)
            )
            if self.use_bias
            else None
        )
