...
try:
    ...
    print("Warning: open_clip not available. CLIP evaluation will be disabled.")
...
class GlideSampler:
    ...
    def _load_base_model(self):
        ...
        if self.config.base_model_path:
            ...
            base_path = os.path.join(os.path.dirname(__file__), "..", "glide_model_cache", "base.pt")
            
        if os.path.exists(base_path):
            print(f"Loading base model from {base_path}")
            self.base_model, self.base_diffusion, self.base_options = load_model(
                glide_path=base_path,
                model_type="base",
                use_fp16=False
            )
            ...
            print(f"Loading base model from OpenAI checkpoint (no cache found at {base_path})")
            self.base_model, self.base_diffusion, self.base_options = load_model(
                model_type="base",
                use_fp16=False
            )
    ...
    def _sample_with_model(self, prompts: List[str], seeds: List[int], model, diffusion, options) -> List[Image.Image]:
...
class EvaluationRunner:
    ...
    def __init__(self, config: EvaluationConfig):
        ...
        if OPEN_CLIP_AVAILABLE:
            ...
            raise ImportError("CLIP evaluation requires open_clip package")
    ...
    def _log_to_wandb(self, 
        ...
        for img_cur, img_base, prompt, score_cur, score_base in zip(
            current_images, base_images, prompts, clip_scores_current, clip_scores_base
        ):
    ...
    def run_evaluation(self, current_model, current_diffusion, current_options, step: int = 0) -> Dict[str, Any]:
        """
        ...
        """
        print(f"Running evaluation with {len(self.prompts)} prompts, {self.config.variations} variations")
        ...
        for variation in range(self.config.variations):
            ...
            self._log_to_wandb(
                current_images, base_images, self.prompts,
                clip_scores_current, clip_scores_base, variation, step
            )
