...
def get_glide_lora_target_modules(model: nn.Module, mode: str = "attention") -> List[str]:
    """
    ...
    for name in linear_layers.keys():
        ...
        if "transformer" in name and ("c_attn" in name or "c_proj" in name or "c_fc" in name):
    ...
    if not target_modules:
        if mode == "minimal":
            ...
            target_modules = [".*transformer.*c_attn.*", ".*transformer.*c_proj.*", ".*attentions.*qkv.*"]
        elif mode == "attention":
...
def apply_lora_to_glide(
    ...
    def glide_forward(self, x, timesteps, tokens=None, mask=None, low_res=None):
        ...
        if low_res is not None:
            return original_forward(x, timesteps, tokens=tokens, mask=mask, low_res=low_res)
        else:
...
class LoRAScheduler:
    ...
    def step(self, step: int):
        ...
        warmup_factor = min(1.0, step / self.warmup_steps) if self.warmup_steps > 0 else 1.0
