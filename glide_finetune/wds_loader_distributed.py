...
from glide_finetune.augmentations import create_laion_augmentor, DiffusionAugmentor
...
def distributed_wds_loader(
    ...
    wds_dataset_name: Optional[str] = None,
    world_size: int = 1,
    rank: int = 0,
    num_workers: int = 4,
    ...
    augmentor: Optional[DiffusionAugmentor] = None,
):
    """
    ...
    def decode_sample(sample):
        ...
        if image_key in sample:
            ...
            raise KeyError(f"Image key '{image_key}' not found in sample")
        ...
        if uncond_p > 0 and random.random() < uncond_p:
            caption = ""
        ...
        if enable_upsample:
            ...
            # Convert to tensors
            low_res_tensor = pil_image_to_norm_tensor(low_res)
            high_res_tensor = pil_image_to_norm_tensor(high_res)
            ...
            if use_augmentations and augmentor is not None:
                ...
                if resize_ratio < 1.0:
                    crop_ratio = random.uniform(resize_ratio, 1.0)
            ...
            # Convert to tensor
            image_tensor = pil_image_to_norm_tensor(image)
    ...
    if enable_upsample:
        dataset = dataset.batched(batch_size, collation_fn=lambda samples: collate_upsample(samples, tokenizer))
    else:
        dataset = dataset.batched(batch_size, collation_fn=lambda samples: collate_base(samples, tokenizer))
...
def collate_base(samples, tokenizer=None):
    """Collate function for base model training."""
    captions, images = zip(*samples)
...
def collate_upsample(samples, tokenizer=None):
    """Collate function for upsampling model training."""
    captions, low_res_images, high_res_images = zip(*samples)
    ...
    # Stack images
    low_res_images = th.stack(low_res_images)
    high_res_images = th.stack(high_res_images)
    
    return tokens, masks, low_res_images, high_res_images
