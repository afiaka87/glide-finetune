...
class MetricsTracker:
    ...
    def __init__(self, window_size: int = 100, enable_clip_eval: bool = False, 
                 clip_eval_config: Optional[Dict[str, Any]] = None,
                 gradient_accumulation_steps: int = 1):
        self.window_size = window_size
        self.rolling_metrics = defaultdict(lambda: RollingAverage(window_size))
        self.step_times = deque(maxlen=window_size)
        self.optimizer_step_times = deque(maxlen=window_size)
    ...
    def update_gradient_stats(self, model: th.nn.Module):
        ...
        for p in model.parameters():
            if p.grad is not None:
                ...
                zero_grads += (grad_data == 0).sum().item()
                
                # Check for NaN gradients
                nan_grads += th.isnan(grad_data).sum().item()
                
                # Check for infinite gradients
                inf_grads += th.isinf(grad_data).sum().item()
    ...
    def update_timing(self, is_optimizer_step: bool = False):
    ...
    def update_batch_size(self, batch_size: int, is_optimizer_step: bool = False):
    ...
    def check_evaluation_milestones(self, current_step: int, use_optimizer_steps: bool = True) -> bool:
        """
        ...
        for milestone in self.eval_milestones:
            if effective_step >= milestone and milestone not in self.completed_evals:
                ...
                print(f"🎯 Reached evaluation milestone: optimizer step {milestone}")
                return True
                
        return False
    
    def maybe_run_evaluation(self, 
                           current_step: int,
                           model, 
                           diffusion, 
                           options, 
                           optimizer=None) -> Optional[Dict[str, Any]]:
        """
    ...
    def add_evaluation_milestone(self, step: int):
        ...
        if step not in self.eval_milestones:
            ...
            print(f"➕ Added evaluation milestone: step {step}")
    
    def remove_evaluation_milestone(self, step: int):
        """Remove an evaluation milestone."""
        if step in self.eval_milestones:
            self.eval_milestones.remove(step)
            print(f"➖ Removed evaluation milestone: step {step}")
    
    def get_next_milestone(self, current_step: int) -> Optional[int]:
    ...
    def update_aesthetic_score(self, images: List[Image.Image]):
        """Calculate and update aesthetic scores for generated images."""
        if not images:
            return
            
        scores = []
        color_stats = {"saturation": [], "brightness": [], "contrast": []}
    ...
    def _sharpness_score(self, img: np.ndarray) -> float:
        ...
        return min(sharpness * 1000, 1.0)  # Scale and cap
    ...
    def _exposure_score(self, img: np.ndarray) -> float:
        ...
        exposure_score = (1.0 - exposure_penalty) * min(contrast * 4, 1.0)
    ...
    def _setup_clip_evaluation(self, config: Dict[str, Any]):
        ...
        try:
            ...
            self.eval_milestones = config.get('eval_milestones', [1000, 2500, 5000, 10000, 15000, 25000])
            
            self.clip_evaluator = create_memory_conscious_evaluator(**default_config)
            print(f"✅ Memory-conscious CLIP evaluation enabled with {len(self.clip_evaluator.prompts)} prompts")
            print(f"   Evaluation milestones: {self.eval_milestones}")
            
        except ImportError as e:
            print(f"⚠️  CLIP evaluation disabled: {e}")
            self.enable_clip_eval = False
        except Exception as e:
            print(f"⚠️  Failed to setup CLIP evaluation: {e}")
            self.enable_clip_eval = False
    
    def run_clip_evaluation(self, model, diffusion, options, optimizer=None, step=0) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            print("🔍 Running memory-conscious CLIP evaluation...")
            ...
            print(f"✅ Memory-conscious CLIP evaluation complete in {eval_results['evaluation_time']:.1f}s")
            print(f"   CLIP Score: {eval_results['clip_score_current']:.4f} (Δ{eval_results['clip_score_delta']:+.4f})")
            print(f"   Win Rate: {eval_results['win_rate_vs_base']:.3f} ({eval_results['wins_count']}/{eval_results['num_prompts']} wins)")
            
            return eval_results
            
        except Exception as e:
            print(f"⚠️  Memory-conscious CLIP evaluation failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def update_clip_metrics(self, clip_results: Dict[str, Any]):
        """Update rolling metrics with CLIP evaluation results."""
        if clip_results is None:
            return
            
        for key in ['clip_score_current', 'clip_score_base', 'clip_score_delta', 'win_rate_vs_base']:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.step_times:
            ...
            metrics["iterations_per_sec"] = 1.0 / avg_iteration_time if avg_iteration_time > 0 else 0.0
        
        if self.optimizer_step_times:
            avg_optimizer_step_time = sum(self.optimizer_step_times) / len(self.optimizer_step_times)
            metrics["optimizer_step_time"] = avg_optimizer_step_time
            metrics["optimizer_steps_per_sec"] = 1.0 / avg_optimizer_step_time if avg_optimizer_step_time > 0 else 0.0
        ...
        for clip_metric in ["clip_score_current", "clip_score_base", "clip_score_delta", "win_rate_vs_base", "clip_eval_time"]:
