...
def glide_wds_loader(
    ...
    words_to_skip=[],
    dataset_name="laion",  # can be laion, alamy, simple, synthetic, or datacomp-clip.
    upscale_factor=4,
    ...
    captions_jsonl=None,  # Path to captions JSONL (required for datacomp-clip)
):
    if debug:
    ...
    # datacomp-clip: load JSONL caption index keyed by sample key
    caption_index = None
    if dataset_name == "datacomp-clip":
        if captions_jsonl is None:
            raise ValueError("datacomp-clip dataset requires --captions_jsonl")
        print(f"Loading caption index from {captions_jsonl} ...")
        caption_index = {}
        with open(captions_jsonl, "r") as f:
            for line in f:
                ...
                if not key:
                    continue
                caption_index[key] = obj
        print(f"  Loaded {len(caption_index):,} entries")
    ...
    def filter_dataset_datacomp_clip(item):
        ...
        if not sample_key or sample_key not in caption_index:
            return False
        # Skip samples missing either CLIP score
        entry = caption_index[sample_key]
    ...
    if dataset_name == "laion":
        ...
        raise ValueError(
            f"Unknown dataset: {dataset_name}. Must be one of 'laion', 'alamy', 'simple', 'synthetic', or 'datacomp-clip'."
        )
    ...
    def preprocess_dataset(item):
        ...
        if not enable_text or random() < uncond_p:
            ...
            if dataset_name == "synthetic":
                ...
                entry = caption_index[item["__key__"]]
