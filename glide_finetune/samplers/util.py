...
def get_karras_sigmas(
    num_steps: int, sigma_min: float, sigma_max: float, rho: float = 7.0
) -> np.ndarray:
...
def wrap_glide_model_for_sampling(
    ...
    def wrapped_model(x: th.Tensor, sigma: float, **model_kwargs) -> th.Tensor:
        ...
        batch_timesteps = th.full((x.shape[0],), timestep, device=device, dtype=th.long)
...
def get_glide_schedule_timesteps(
    num_steps: int, num_timesteps: int, sigmas: np.ndarray, use_karras: bool = False
    ...
    if use_karras:
        ...
        timesteps = np.linspace(num_timesteps - 1, 0, num_steps, dtype=np.int64)
