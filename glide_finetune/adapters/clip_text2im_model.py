...
from glide_text2im.unet import AttentionBlock, TimestepEmbedSequential, TimestepBlock

from .clip_adapter import ClipAdapter, ClipTextEncoder
from .dual_attention import replace_attention_blocks, DualConditioningAdapter, DualAttentionBlock
...
class ClipText2ImUNet(Text2ImUNet):
    ...
    def _setup_clip_conditioning(
        ...
        from .clip_adapter import load_clip_model, create_clip_adapter_config
    ...
    def forward(
        ...
        if dry_run:
            use_clip_for_forward = False
        else:
            use_clip_for_forward = use_clip
        ...
        if use_clip and (clip_text_prompts is not None or clip_embeddings is not None):
            ...
            # print(f"DEBUG forward: use_clip={use_clip}, clip_out shape={clip_out.shape if clip_out is not None else None}")
            # print(f"DEBUG forward: adapter gate={self.clip_adapter.get_gate_value():.4f}")
        ...
        for module in self.input_blocks:
            h = self._forward_timestep_embed_sequential(module, h, emb, xf_out, clip_out)
            hs.append(h)
        
        h = self._forward_timestep_embed_sequential(self.middle_block, h, emb, xf_out, clip_out)
    ...
    def get_adapter_params(self):
        """Get parameters for the CLIP adapter components only."""
        params = []
        ...
        # Also include dual attention gates
        for module in self.modules():
            if hasattr(module, 'clip_gate'):
                params.append(module.clip_gate)
            if hasattr(module, 'clip_kv'):
                params.extend(module.clip_kv.parameters())
            if hasattr(module, 'clip_attention'):
                params.extend(module.clip_attention.parameters())
    ...
    def set_adapter_gate_schedule(self, current_step: int, warmup_steps: int):
        ...
        for module in self.modules():
            if hasattr(module, 'set_clip_gate_value'):
    ...
    def dry_run_test(
        ...
        if self.use_clip and (clip_text_prompts is not None or clip_embeddings is not None):
            with torch.no_grad():
        ...
        if return_metrics and dry_run_output is not None:
            ...
            results['outputs_identical'] = torch.allclose(dry_run_output, baseline_output, rtol=1e-5, atol=1e-8)
            
            # Add adapter metrics
            if hasattr(self, 'clip_adapter'):
                results['adapter_gate_value'] = self.clip_adapter.get_gate_value()
