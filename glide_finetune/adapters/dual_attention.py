...
from glide_text2im.nn import conv_nd
...
class DualAttentionBlock(AttentionBlock):
    ...
    def __init__(
        ...
        if clip_channels is not None:
            ...
            if clip_gate_init == 0.0:
                ...
                gate_logit = torch.log(
                    torch.tensor(clip_gate_init / (1 - clip_gate_init))
                ).item()
    ...
    def forward(self, x, encoder_out=None, clip_encoder_out=None):
        ...
        if is_cross_attention and (
            encoder_out is not None or clip_encoder_out is not None
        ):
            ...
            if encoder_out is not None and clip_encoder_out is not None:
                ...
                if hasattr(self, "_debug") and self._debug:
                    print(
                        f"  DualAttentionBlock cross-attention: "
                        f"gate={gate.item():.4f}, "
                        f"h_text norm={h_text.norm().item():.4f}, "
                        f"h_clip norm={h_clip.norm().item():.4f}"
                    )
            ...
            if clip_encoder_out is not None and hasattr(self, "clip_gate"):
                ...
                if hasattr(self, "_debug") and self._debug:
                    print(
                        f"  DualAttentionBlock self-attention: "
                        f"gate={gate.item():.4f}, "
                        f"h_self norm={h_self.norm().item():.4f}, "
                        f"h_clip norm={h_clip.norm().item():.4f}"
                    )
            else:
                # Pure self-attention
...
def replace_attention_blocks(model, clip_channels: int, clip_gate_init: float = 0.0):
    ...
    def replace_module(parent, name, module):
        if isinstance(module, AttentionBlock) and not isinstance(
