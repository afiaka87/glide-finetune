...
class ModelMemoryManager:
    ...
    def safe_delete_model(self, model: th.nn.Module, model_name: str = "model"):
        ...
        print(f"🗑️  Unloading {model_name}...")
    ...
    def load_model_safely(self, 
        ...
        initial_snapshot = self.monitor.log_memory_snapshot(f"Before loading {model_name}")
        ...
        if final_snapshot['memory_usage_pct'] > self.max_memory_usage_pct:
            print(f"⚠️  Warning: Memory usage ({final_snapshot['memory_usage_pct']:.1f}%) "
...
@contextmanager
def temporary_model_load(memory_manager: ModelMemoryManager,
                        loader_func,
                        model_name: str,
                        *args,
                        **kwargs):
...
class ModelStateManager:
    ...
    def save_model_state(self, 
        ...
        print(f"💾 Saving model state: {state_name}")
        ...
        print(f"✅ Saved {state_name} ({self.saved_states[state_name]['size_mb']:.1f} MB)")
    ...
    def restore_model_state(self,
        ...
        print(f"📂 Restoring model state: {state_name}")
        ...
        try:
            ...
            print(f"✅ Restored {state_name}")
            return True
            
        except Exception as e:
            print(f"❌ Failed to restore {state_name}: {e}")
            return False
    ...
    def cleanup_states(self, keep_latest: int = 1):
        ...
        for state_name, info in sorted_states[keep_latest:]:
            try:
                ...
                print(f"🗑️  Cleaned up old state: {state_name}")
            except Exception as e:
                print(f"⚠️  Failed to cleanup {state_name}: {e}")
