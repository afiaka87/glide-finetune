...
from .master_weight_manager import MasterWeightManager, GradientAccumulator
...
class FP16TrainingStep:
    ...
    def _setup_master_weights(self, optimizer: torch.optim.Optimizer):
    ...
    def training_step(self, 
                     compute_loss: Callable[[], torch.Tensor]) -> Dict[str, Any]:
        """
        ...
        # Check for NaN
        if torch.isnan(loss):
            if self.nan_recovery and self.nan_recovery.can_recover():
                self.logger.warning("NaN detected, attempting recovery")
                self.nan_recovery.recover(self.model, self.optimizer)
                ...
                raise ValueError("NaN detected and recovery failed")
...
class SelectiveFP16Converter:
    ...
    def convert_model_mixed_precision(self, 
                                     model: nn.Module) -> Tuple[nn.Module, Dict[str, int]]:
        """
        ...
        if total_params > 0:
            ...
            self.logger.info(
                f"Model conversion complete: {fp16_ratio:.1f}% FP16, "
                f"{100-fp16_ratio:.1f}% FP32"
            )
            ...
            if param.dtype == torch.float32:
                ...
                for critical in ['norm', 'embed', 'time', 'label', 'final']:
                    if critical in name.lower():
                        should_convert = False if not self.aggressive else True
                        break
