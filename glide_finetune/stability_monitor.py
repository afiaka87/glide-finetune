...
class StabilityMonitor:
    ...
    def __init__(
        ...
        self.baseline_loss_history: Deque[float] = deque(maxlen=window_size)

        # Track KL divergence
        self.kl_history: Deque[float] = deque(maxlen=window_size)

        # Track gate values
        self.gate_history: Deque[float] = deque(maxlen=window_size)

        # Track gradient norms
        self.adapter_grad_history: Deque[float] = deque(maxlen=window_size)
        self.main_grad_history: Deque[float] = deque(maxlen=window_size)

        # Stability test results
        self.stability_test_results: List[Dict[str, Any]] = []
    ...
    def check_for_issues(self) -> Dict[str, Any]:
        ...
        if (
            loss_std > 0
            and abs(recent_losses[-1] - loss_mean) > self.spike_threshold * loss_std
        ):
            issues["has_issues"] = True
            issues["loss_spike"] = True
            recommendations.append(
                "Loss spike detected. Consider reducing learning rate."
            )
        ...
        if len(self.loss_history) == self.window_size:
            ...
            if recent_mean > early_mean * (1 + self.degradation_threshold):
                ...
                recommendations.append(
                    "Loss increasing over time. Consider checkpoint rollback."
                )
        ...
        if len(self.kl_history) > 10:
            ...
            if kl_mean > 1.0:  # High KL divergence
                ...
                recommendations.append(
                    "High KL divergence. Reduce adapter learning rate or gate warmup."
                )
        ...
        if len(self.adapter_grad_history) > 5:
            ...
            if any(g > 10.0 for g in recent_grads):
                ...
                recommendations.append(
                    "Gradient explosion in adapter. Reduce gradient clipping threshold."
                )
    ...
    def run_stability_test(
        ...
        with torch.no_grad():
            ...
            identical = torch.allclose(
                model_output, baseline_output, rtol=1e-5, atol=1e-8
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if self.adapter_grad_history:
            ...
            grad_stats["adapter"] = {
                ...
                "recent_mean": np.mean(adapter_grads[-10:])
                if len(adapter_grads) >= 10
                else np.mean(adapter_grads),
            }
        ...
        if self.main_grad_history:
            ...
            grad_stats["main"] = {
                ...
                "recent_mean": np.mean(main_grads[-10:])
                if len(main_grads) >= 10
                else np.mean(main_grads),
            }

        summary["grad_stats"] = grad_stats
    ...
    def format_summary(self) -> str:
        ...
        if summary["kl_stats"]:
            lines.append(
                f"KL Divergence: {summary['kl_stats']['recent_mean']:.4f} "
        ...
        if "adapter" in summary["grad_stats"]:
            lines.append(
                f"Adapter Gradients: "
                f"{summary['grad_stats']['adapter']['recent_mean']:.4f}"
            )
