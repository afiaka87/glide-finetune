...
class DynamicLossScaler:
    ...
    def update_scale(self, overflow: bool) -> None:
        ...
        if overflow:
            ...
            self._scale = max(self._scale / self.config.scale_factor, 
            ...
            if self._unskipped >= self.config.scale_window:
                ...
                self._scale = min(self._scale * self.config.scale_factor,
    ...
    def get_grad_norm(self, optimizer: torch.optim.Optimizer, 
    ...
    def log_stats(self) -> None:
        ...
        avg_grad_norm = (sum(self.stats['gradient_norms']) / len(self.stats['gradient_norms'])
                        if self.stats['gradient_norms'] else 0)
        
        self.logger.info(f"Loss Scaler Stats: scale={self._scale:.1f}, "
...
class NaNRecoverySystem:
    ...
    def save_good_state(self, model: nn.Module, 
                       optimizer: torch.optim.Optimizer,
                       loss_scaler: DynamicLossScaler) -> None:
    ...
    def attempt_recovery(self, model: nn.Module,
                         optimizer: torch.optim.Optimizer,
                         loss_scaler: DynamicLossScaler) -> bool:
        """
...
def test_loss_scaler():
    ...
    print("Testing Dynamic Loss Scaler...")
    ...
    for step in range(20):
        ...
        if stepped:
            print(f"Step {step}: scale={scaler.scale:.1f}, grad_norm={scaler.get_grad_norm(optimizer):.2e}")
        else:
            print(f"Step {step}: SKIPPED due to overflow, scale={scaler.scale:.1f}")
    
    # Print final stats
    print(f"\nFinal Statistics:")
    print(f"  Total steps: {scaler.stats['total_steps']}")
    print(f"  Skipped steps: {scaler.stats['skipped_steps']}")
    print(f"  Scale updates: {scaler.stats['scale_updates']}")
    print(f"  Final scale: {scaler.scale}")
