...
def randomize_linear_weight(weight: torch.Tensor, std: Optional[float] = None) -> None:
    """
...
def randomize_conv_weight(weight: torch.Tensor, std: Optional[float] = None) -> None:
    """
...
def randomize_module_weights(
    ...
    if isinstance(module, nn.Parameter):
        ...
        if verbose:
            print(f"    Randomized parameter {module_name} ({layer_type}): {params_randomized:,} params")
    ...
    if verbose and params_randomized > 0:
        print(f"    Randomized module {module_name}: {params_randomized:,} params total")
...
def randomize_transformer(
    ...
    summary = apply_to_selected_components(
        model,
        mode="transformer", 
        operation_fn=randomize_op,
        operation_name="randomize"
    )
...
def verify_randomization(
    ...
    for param_name, param in current_state.items():
        ...
        should_be_randomized = any(
            param_name.startswith(comp_name)
            for comp_name in target_components.keys()
        )
        
        if should_be_randomized:
            if param_name in original_state:
                if not torch.equal(param, original_state[param_name]):
                    any_changed = True
                    break
    
    if not any_changed:
        print(f"WARNING: No parameters were changed during {mode} randomization!")
        return False
    ...
    for param_name, param in current_state.items():
        should_be_unchanged = any(
            param_name.startswith(comp_name)
            for comp_name in unchanged_components.keys()
        )
        
        if should_be_unchanged and param_name in original_state:
            if not torch.equal(param, original_state[param_name]):
                print(f"WARNING: Parameter {param_name} changed but shouldn't have!")
                return False
