...
class MemoryConstrainedClipEvaluator:
    ...
    def __init__(self, config: MemoryConstrainedEvalConfig):
        ...
        self.memory_manager = ModelMemoryManager(
            ...
            max_memory_usage_pct=config.max_memory_usage_pct
        )
        ...
        print(f"🧪 Memory-Conscious CLIP Evaluator initialized")
        print(f"   Prompts: {len(self.prompts)}")
        print(f"   Variations: {config.variations}")
        print(f"   Memory limit: {config.max_memory_usage_pct}%")
    ...
    def _load_prompts(self) -> List[str]:
        ...
        if len(prompts) < self.config.max_prompts:
            print(f"Warning: Only {len(prompts)} prompts available, expected {self.config.max_prompts}")
    ...
    def _load_clip_model(self):
        ...
        if not OPEN_CLIP_AVAILABLE:
            raise ImportError("open_clip is required for CLIP evaluation")
        ...
        return self.memory_manager.load_model_safely(
            clip_loader, "clip_model"
        )
    
    def _score_images_with_clip(self, 
                               prompts: List[str], 
                               images: List[Image.Image],
                               phase_name: str) -> th.Tensor:
        """Score images using CLIP with memory management."""
        print(f"🔍 CLIP scoring for {phase_name}...")
        
        with temporary_model_load(
            self.memory_manager, 
            self._load_clip_model, 
            "clip_scorer"
        ) as (clip_model, preprocess, tokenizer):
    ...
    def _sample_from_model(self, 
                          model, 
                          diffusion, 
                          options,
                          prompts: List[str], 
                          seeds: List[int],
                          model_name: str) -> List[Image.Image]:
    ...
    def _load_base_model(self):
        ...
        def base_loader():
            if self.config.base_model_path and os.path.exists(self.config.base_model_path):
                ...
                return load_model(
                    glide_path=self.config.base_model_path,
                    model_type="base",
                    use_fp16=False
                )
            else:
                print("Loading base model from OpenAI checkpoint")
                return load_model(
                    model_type="base",
                    use_fp16=False
                )
    ...
    def run_evaluation(self, 
        ...
        """
        print("🚀 Starting memory-conscious CLIP evaluation...")
        ...
        try:
            ...
            self.current_phase = EvaluationPhase.SAVING_TRAINING_STATE
            print(f"📁 Phase 1: {self.current_phase.value}")
            
            training_state_name = f"training_state_step_{step}"
            self.state_manager.save_model_state(
                current_model, optimizer, training_state_name
            )
            ...
            for variation in range(self.config.variations):
                ...
                print(f"📁 Phase 3: {self.current_phase.value}")
                ...
                print(f"📁 Phase 4: {self.current_phase.value}")
                ...
                with temporary_model_load(
                    ...
                    print(f"📁 Phase 5: {self.current_phase.value}")
                    
                    base_images = self._sample_from_model(
                        base_model, base_diffusion, base_options,
                        self.prompts, seeds, "base_model"
                    )
                ...
                print(f"📁 Phase 6: {self.current_phase.value}")
                
                base_scores = self._score_images_with_clip(
                    self.prompts, base_images, "base model"
                )
            ...
            print(f"📁 Phase 7: {self.current_phase.value}")
            ...
            print(f"📁 Phase 8: {self.current_phase.value}")
            ...
            if not success:
                print("⚠️  Warning: Failed to restore training state")
            else:
            ...
            print("✅ Memory-conscious evaluation complete!")
            print(f"   Evaluation time: {eval_time:.1f}s")
            print(f"   CLIP Score Current: {metrics['clip_score_current']:.4f}")
            print(f"   CLIP Score Base: {metrics['clip_score_base']:.4f}")
            print(f"   Win Rate: {metrics['win_rate_vs_base']:.3f}")
            
            # Final memory report
            print(self.memory_manager.memory_report())
            ...
            print(f"❌ Evaluation failed during {self.current_phase.value}: {e}")
            ...
            try:
                ...
                print("❌ Failed to restore training state after error")
            
            raise e
    
    def _log_variation_to_wandb(self,
        ...
        for img_cur, img_base, prompt, score_cur, score_base in zip(
            current_images, base_images, self.prompts, current_scores, base_scores
        ):
    ...
    def _calculate_metrics(self, current_scores: List[th.Tensor], base_scores: List[th.Tensor]) -> Dict[str, Any]:
